(function(capt) {
	$(document)
			.ready(
					function() {
						// classe Acao
						function Acao(acao, tempo, url, conteudo, tag, tagId,
								tagClass, tagName, tagValue, tagType,
								posicaoPaginaX, posicaoPaginaY) {
							this.tipoAcao = acao;
							this.tempo = tempo;
							this.url = url;
							this.conteudo = htmlEntities(conteudo).substr(0,
									200);
							this.tag = tag;
							this.tagId = tagId;
								...
								.bind(
										{
											click : function(e) {
												...
												if (tagName == "TD") {
													...
													// conteúdo interno do
													// elemento clicado,
													// adiciona uma nova div por
													// html() ou prepend()
													var celulaInicial = capt(
															e.target).html()
													var linhaInicial = capt(
															e.target).parent()
															.html()

													// adiciono itemClicado na
													// celula clicada e
													// trClicado no inicio
													// da linha clicada
													capt(e.target)
												...
												var href;
												// no caso de links que possuem
												// conteúdos internos
												var tagPaiClick = capt(e.target)
														.parent();
						...
						/*
						 * Método para percorrer o html passado, observando a
						 * qual linha/coluna o objeto pertence
						 */
						...
						function enviaJson(dados, bool) {
							...
							/*
							 * capt.ajax({ url:
							 * "/Usabilidade/tarefa/save/fluxo/ideal", type:
							 * "POST", data: { 'data': eval(dados) }, dataType:
							 * "html" }).done(function( msg ) { alert("Dados: " +
							 * msg); });
							 */
							// alert(dados);
