...
//	var window = {};
	...
	capt.webclientCapt = function(settings) {
		capt(document).ready(function() {
							...
							if (settings){capt.extend(config, settings);}
			
							// classe Acao
							function Action(acao, tempo, url, conteudo, tag, tagId, tagClass, tagName, tagValue, tagType, posX, posY) {
								this.sActionType = acao;
								this.sTime = tempo;
								this.sUrl = url;
								this.sContent = htmlEntities(conteudo).substr(0, 200);
								this.sTag = tag;
								this.sTagId = tagId;
								this.sTagClass = tagClass;
								this.sTagName = tagName;
								this.sTagValue = tagValue;
								this.sTagType = tagType;
								this.sPosX = posX;
								this.sPosY = posY;
							}
							...
							capt(document).bind({
												click : function(e) {
													...
													if (tagName == "TD") {
														...
														// .parent().tagName não deu -> tabelaClicada[0].nodeName recebendo TABLE
														...
														Acoes.push(
															new Action("click",
																...
																htmlEntities(celulaInicial).substr(0,200),
																"["+ posicaoTabela+ "] "+ tagName+ " > TABLE",
																retira_(capt(e.target).attr("id"))+ " > "+ retira_(tabelaClicada.attr("id")),
																retira_(capt(e.target).attr("class"))+ " > "+ retira_(tabelaClicada.attr("class")),
																retira_(capt(e.target).attr("name"))+ " > "+ retira_(tabelaClicada.attr("name")),
																retira_(capt(e.target).attr("value"))+ " > "+ retira_(tabelaClicada.attr("value")),
																retira_(capt(e.target).attr("type"))+ " > "+ retira_(tabelaClicada.attr("type")),
																e.pageX,
																e.pageY)
														);
