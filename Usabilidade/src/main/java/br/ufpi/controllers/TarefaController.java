...
public class TarefaController {
    ...
    public TarefaController(Result result, TarefaRepository tarefaRepository,
            ...
            SessionActions actions, SessionFluxoTarefa fluxoTarefa, HttpServletRequest request) {
        super();
        this.result = result;
    ...
    public void salvarTarefa(Tarefa tarefa) {
        ...
        result.redirectTo(TesteController.class).passo1(
     ...
     * Testador
     * 
     * @param idTarefa identicado da Tarefa a ser realizada
     * @param url url a ser exibida
     * @return
     */
    @Logado
    @Get()
...
//			result.include("url", tarefa.getUrlInicial());
    ...
    private void gravaFluxoIdeal(Long tarefaId) {
        Tarefa tarefa = this.tarefaPertenceTeste(usuarioLogado.getTeste().getId(), tarefaId);
    ...
    private Tarefa tarefaPertenceTesteNaoRealizado(Long tarefaId) {
        Tarefa tarefaRetorno = tarefaRepository.perteceTesteNaoRealizado(
                tarefaId, usuarioLogado.getTeste().getId(), usuarioLogado.getUsuario().getId());
