...
import br.ufpi.datamining.analisys.fuzzy.Matrix.Builder;
...
public class FuzzyCMeansAlgorithm {
	 ...
	 * @param initFeatures
	 */
	public FuzzyCMeansAlgorithm(int nbCluster, double m, List<Point> initFeatures, double toleranceLevel, double[][] matrix) {
		this.m = m;
		this.iteration = 0;
	...
	private void updatePoints() {
		...
		for(int col = 0; col < this.features.size(); col++){
			double[] relevance = new double[this.nbCluster];
			/* loop over clusters */
			for(int line = 0; line < this.nbCluster; line++){
				relevance[line] = this.matrixU.valueAt(line, col);
			}
			this.features.get(col).setRelevance(relevance);
		}
	}
	 ...
	 * @return
	 */
	private static double euclideanDistance(Point a, double[] centroid) {
		double sum = 0.0;
		sum += Math.pow(a.getX() - centroid[0], 2);
		sum += Math.pow(a.getY() - centroid[1], 2);
